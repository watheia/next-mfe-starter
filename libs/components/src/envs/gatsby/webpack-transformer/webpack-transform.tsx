...
export function gatsbyTransform(config: WebpackConfigMutator) {
  ...
  const newNodeModulesPattern = RegExp(
    generateNodeModulesPattern({ packages: ['gatsby'] })
  );

  const updatedExcludes = babelLoaders[0].exclude.map((it: any) => {
    if (RegExp(it).test('node_modules')) return newNodeModulesPattern;
    return it;
  });
